...
<template>
    <div class="flex flex-col">
        ...
        <input id="input" @change="props.atChange" :type="props.type"
            @input="$emit('update:modelValue', $event.target.value)" :accept="props.accept"
            :class="`${props.classes} text-gray-600 border ${currentTheme.borderColor} ${props.fileName ? 'rounded-b-none' : ''} file:border-gray-300 ${InputTypeFileBorder} focus:outline-0 bg-transparent text-sm rounded-lg w-full p-2.5  file:mr-3 file:p-0.5 file:text-sm file:font-medium file:bg-gray-50 file:text-gray-600 hover:file:cursor-pointer file:rounded-md hover:file:bg-blue-50 hover:file:text-blue-700 file:w-[100%]`"
            required />
    </div>
