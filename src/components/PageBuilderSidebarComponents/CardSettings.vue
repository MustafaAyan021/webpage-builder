...
<template>
    <div id="card-editing" v-show="sidebarContent.openCardEditing">
        ...
        <div class="flex flex-col gap-1">
            ...
            <p v-show="activeInputAlert && cardHeading == ''" class="text-sm text-red-600 ml-3"> Please fill the folloing field</p>
            <Textarea v-model="cardParagraph" label="Paragraph" classes="h-44" maxlength="80" />
            <p v-show="activeInputAlert && cardParagraph == ''" class="text-sm text-red-600 ml-3"> Please fill the folloing field</p>
            <Button type="submit" variant="solid" @click="handleItemSubmit">Add Item</Button>
        </div>
        <h1 v-show="childrenCards.length > 0" :class="`${currentTheme.text} font-bold text-lg ml-1 mt-3`">Edit Cards</h1>
